    ...
    public partial class Inventory
    {
        ...
        public void UseItem(InventoryItem item, bool fromInventory = true)
        {
            ...
            else
            {
                ...
                else if (usableType == UsableType.Custom)
                {
                    if (item.Item.UsableSettings.removeOnUse)
                        RemoveItem(item, 1);
                }

                if (item.Item.Settings.applyAttributesOnUse)
                {
                    if (item.CustomData.Type == ItemCustomData.DataType.Attributes)
                    {
                        PlayerManager pm = playerPresence.PlayerManager;
                        foreach (var attr in item.CustomData.ItemAttributes.Items)
                        {
                            if (attr is not PlayerAttribute playerAttr)
                                continue;
            ...
            if (fromInventory) gameManager.ShowInventoryPanel(false);
        }
