...
namespace UHFPS.Runtime
{
    ...
    public class GameManager : Singleton<GameManager>
    {
        ...
        public enum GamePauseType { KeepGame, FreezeGame }
        ...
        public GamePauseType PauseType = GamePauseType.KeepGame;
        ...
        // Equipment Hint Controls
        public GameObject EquipmentControl;
        public Transform EquipmentControlParent;
        ...
        private Coroutine examineCoroutine;
        ...
        private readonly Dictionary<GameObject, ControlInfo> itemControls = new();
        ...
        {
            get => PlayerPresence.PlayerManager.PlayerHealth.IsDead;
        }
        ...
        /// <summary>
        /// Load last saved world state of the current scene.
        /// </summary>
        ...
        private IEnumerator LoadLast()
        {
            yield return BackgroundFade.StartBackgroundFade(false);
            yield return new WaitForEndOfFrame();

            SaveGameManager.SetLoadWorldLast();
        ...
        /// Show hint controls.
        /// </summary>
        public void ShowHintControls(params ControlInfo[] controls)
        {
            foreach (var control in controls)
            {
                if (!control.Input.IsAssigned || string.IsNullOrEmpty(control.Text))
                    continue;

                GameObject ctr = Instantiate(EquipmentControl, EquipmentControlParent);
                ItemControlInfo ctrInfo = ctr.GetComponent<ItemControlInfo>();
                ctrInfo.SetControlInfo(control);

                ctr.SetActive(control.IsEnabled);
                itemControls.Add(ctr, control);
            }
        }
        ...
        /// </summary>
        public void UpdateHintControls()
        {
            foreach (var control in itemControls)
            {
                control.Key.SetActive(control.Value.IsEnabled);
            }
        }

        /// <summary>
        /// Hide instantiated hint controls.
        /// </summary>
        public void HideHintControls()
        {
            foreach (var control in itemControls)
                Destroy(control.Key);
        ...
        {
            if (examineCoroutine != null)
                StopCoroutine(examineCoroutine);

            if (!noFade)
            {
                ExamineText.text = examineText;
                examineCoroutine = StartCoroutine(CanvasGroupFader.StartFade(ExamineInfoPanel, show, ExamineFadeSpeed, () =>
                {
        ...
        public void ShowInventoryPanel(bool state)
        {
            if (PlayerPresence.IsUnlockedAndCamera && !PlayerDied && !isInputLocked && !IsPaused)
            {
                ...
                if (IsInventoryShown)
                {
                    ...
                    if(dialogueSystem != null) dialogueSystem.ForceShow(false);
                }
                else
                {
                    ...
                    if (dialogueSystem != null) dialogueSystem.ForceShow(true);
                }
            }
        }
        ...
        public void ShowHintMessage(string text, float time)
        {
            ...
            messageCoroutine = StartCoroutine(ShowHintMessage(time));
        }
        ...
        public void ResumeGame()
        {
            ...
            // set pause time scale
            if (PauseType == GamePauseType.FreezeGame)
                Time.timeScale = 1f;
        }
